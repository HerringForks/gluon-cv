...
def parse_args():
    ...
    parser.add_argument('--horovod', action='store_true',
                        help='Use horovod MXNet for distributed training. Must be run with OpenMPI. '
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, ctx):
    ...
    train_sampler = \
        gcv.data.sampler.SplitSortedBucketSampler(train_dataset.get_im_aspect_ratio(),
                                                batch_size,
                                                num_parts=dist.size() if args.horovod else 1,
                                                part_index=dist.rank() if args.horovod else 0,
                                                shuffle=True)
...
def get_dali_dataset(dataset_name, devices, args):
    if dataset_name.lower() == "coco":
        ...
        # validation
        if (not args.horovod or dist.rank() == 0):
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            if (not args.horovod or dist.rank() == 0):
                local_batch_size = int(args.batch_size // (dist.size() if args.horovod else 1))
...
if __name__ == '__main__':
    ...
    if args.dali:
        ...
        batch_size = (args.batch_size // dist.size()) if args.horovod else args.batch_size
