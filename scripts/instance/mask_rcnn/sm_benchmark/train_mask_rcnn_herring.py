...
def validate(net, val_data, async_eval_processes, ctx, eval_metric, logger, epoch, best_map, args):
    ...
    for ib, batch in enumerate(val_data):
        ...
        for det_bbox, det_id, det_score, det_mask, det_info in zip(det_bboxes, det_ids, det_scores,
                                                                   det_masks, det_infos):
            #print('det info: ', det_info.shape)
            #print('bbox shape: ', det_bbox.shape)
            if det_info.shape != (1, 3):
                print('det info: ', det_info.shape)
            if det_bbox.shape != (1, 1000, 4):
                print('bbox shape: ', det_bbox.shape)
...
if __name__ == '__main__':
    ...
    async_eval_processes = []
    best_map = [0]
    validate(net, val_data, async_eval_processes, ctx, eval_metric, logger, 13, best_map, args)